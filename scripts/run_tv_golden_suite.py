    ...
    for case in cases:
        ...
        if status == "oracle_verified":
            if not str(case.get("oracle_source", "")).startswith("TradingView"):
                raise SystemExit(
                    f"Oracle-verified case {case_id!r} must name a TradingView oracle_source"
                )
            _require_files(
                case_dir,
                case_id,
                case.get("required_files", []),
            ...
            present_required = [
                name for name in case.get("required_files", []) if (case_dir / str(name)).exists()
            ]
            if present_required:
                raise SystemExit(
