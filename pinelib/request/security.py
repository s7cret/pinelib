...
def security(
    ...
    *,
    ...
    if isinstance(expression_callable, Sequence) and not callable(expression_callable):
        ...
        if len(requested_values) != len(requested_bars):
            raise PineRequestError(
                "request.security value sequence length must match requested bars"
            )
    else:
    ...
    # strict_5x returns the current chart bar's merged HTF value.
    profile = getattr(getattr(runtime, "config", None), "semantic_profile", "legacy_4x")
    if profile == "strict_5x":
